...
describe('ReferralEngine', () => {
  ...
  it('generates a 6-character referral code from the expected alphabet', async () => {
  ...
  it('registers a user with no referral code as a root node', async () => {
    const { referral } = setup();
    const user = await referral.registerUser({ phoneNumber: '+98-A' });
  ...
  it('rejects registering the same phone number twice', async () => {
    const { referral } = setup();
    await referral.registerUser({ phoneNumber: '+98-dup' });
    await expect(referral.registerUser({ phoneNumber: '+98-dup' })).rejects.toThrow();
  });

  it('applies a referral code at registration and rejects reusing/forging one afterwards', async () => {
    const { referral } = setup();
    const parent = await referral.registerUser({ phoneNumber: '+98-parent' });
  ...
  it('rejects an invalid referral code and self-referral', async () => {
    const { referral } = setup();
    const user = await referral.registerUser({ phoneNumber: '+98-solo' });
    await expect(referral.applyReferral(user.id, 'ZZZZZZ')).rejects.toThrow();
  ...
  it('grants the registration reward to the direct referrer only, once phone is verified, and never twice', async () => {
    ...
    const parent = await referral.registerUser({ phoneNumber: '+98-p2' });
  ...
  it('distributes subscription rewards to exactly three ancestor levels, matching the PRD example', async () => {
    const { referral, wallet } = setup();
    const a = await referral.registerUser({ phoneNumber: '+98-A' });
  ...
  it('never rewards a fourth ancestor level, even when the tree is deeper', async () => {
    const { referral, wallet } = setup();
    const a = await referral.registerUser({ phoneNumber: '+98-A2' });
  ...
  it('a root node (no referrer) generates no subscription rewards for anyone', async () => {
    const { referral, wallet } = setup();
    const solo = await referral.registerUser({ phoneNumber: '+98-solo2' });
  ...
  it('tags rewards granted under an active campaign override as CAMPAIGN_REWARD', async () => {
    ...
    const a = await referral.registerUser({ phoneNumber: '+98-A3' });
  ...
  it('reports referral tree size and statistics', async () => {
    const { referral } = setup();
    const a = await referral.registerUser({ phoneNumber: '+98-A4' });
